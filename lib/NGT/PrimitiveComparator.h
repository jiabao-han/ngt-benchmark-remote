...
public:
    ...
    inline static double compareL2(const float16* a, const float16* b, size_t size)
    {
        ...
        double s = _mm_cvtss_f32(_mm_shuffle_ps(tmp, tmp, _MM_SHUFFLE(0, 0, 0, 0))) + _mm_cvtss_f32(_mm_shuffle_ps(tmp, tmp, _MM_SHUFFLE(0, 0, 0, 1)));
        return sqrt(s);
        // return s;
    }
#endif
    ...
    inline static double compareL2(const bfloat16* a, const bfloat16* b, size_t size)
    {
        ...
        __m128 sum128 = _mm_add_ps(_mm256_extractf128_ps(sum256, 0), _mm256_extractf128_ps(sum256, 1));
        __m128 tmp = _mm_hadd_ps(sum128, _mm_set1_ps(0));
        double d = _mm_cvtss_f32(_mm_shuffle_ps(tmp, tmp, _MM_SHUFFLE(0, 0, 0, 0))) + _mm_cvtss_f32(_mm_shuffle_ps(tmp, tmp, _MM_SHUFFLE(0, 0, 0, 1)));
        // return sqrt(d);
        return d;
    }
#endif
    ...
    inline static double compareDotProduct(const float16* a, const float16* b, size_t size)
    {
        ...
        __m512 sum512 = _mm512_setzero_ps();
        while (a < last) {
            sum512 = _mm512_add_ps(sum512, _mm512_mul_ps(_mm512_cvtph_ps(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(a))), _mm512_cvtph_ps(_mm256_loadu_si256(reinterpret_cast<const __m256i*>(b)))));
        ...
        __m256 sum256 = _mm256_setzero_ps();
        while (a < last) {
            sum256 = _mm256_add_ps(sum256, _mm256_mul_ps(_mm256_cvtph_ps(_mm_loadu_si128(reinterpret_cast<const __m128i*>(a))), _mm256_cvtph_ps(_mm_loadu_si128(reinterpret_cast<const __m128i*>(b)))));
            a += 8;
            b += 8;
        }
        __m128 sum128 = _mm_add_ps(_mm256_extractf128_ps(sum256, 0), _mm256_extractf128_ps(sum256, 1));
