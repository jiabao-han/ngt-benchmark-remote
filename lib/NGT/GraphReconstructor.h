...
#pragma omp parallel for
  ...
  static void removeShortcutEdges(NGT::GraphIndex &outGraph, int &removeCount, float range, size_t nOfThreads = 0)
  {
...
#pragma omp parallel for
  ...
  static
    void reconstructGraph(std::vector<NGT::ObjectDistances> &graph, NGT::GraphIndex &outGraph, size_t originalEdgeSize, size_t reverseEdgeSize, int64_t maxNoOfEdges = std::numeric_limits<int64_t>::max())
  {
    std::cerr << "GraphReconstructor::reconstructGraph:" << originalEdgeSize << ":" << reverseEdgeSize << ":" << maxNoOfEdges << std::endl;
