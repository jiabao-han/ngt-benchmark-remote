...
void buildIndex(const std::vector<std::vector<float>>& data, const char* index_name)
{
    ...
    if (!ngt_create_index(index, 100, err)) {
        std::cerr << "Error creating index: " << ngt_get_error_string(err) << std::endl;
        ngt_close_index(index);
        ngt_destroy_property(prop);
        ngt_destroy_error_object(err);
    ...
    std::cerr << "Closing the quantized index..." << std::endl;
...
//        }
        ...
        std::string index_path = std::string(index_dir) + "qg-test/gist-test/";
        std::string result_file_path = std::string(result_dir) + "qg-test/gist/gist_recall_qps_result.csv";
